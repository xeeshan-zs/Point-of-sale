let script = function () {
    ...
    this.registerEvents = function () {
        ...
        // Change - using 'input' for real-time sync
        document.addEventListener('input', function (e) {
            ...
            if (targetEl.classList.contains('qtyInput')) {
                let pid = targetEl.dataset.id;
                let newQty = parseInt(targetEl.value);

                if (isNaN(newQty) || newQty < 1) {
                    // Optional: reset to 1 or handle invalid input
                    // newQty = 1; 
                    return;
                }

                // Check stock (simplified logic: assume we can update if stock allows)
                // We need to calculate the difference to update stock correctly
                let oldQty = loadScript.orderItems[pid]['orderQty'];
                let diff = newQty - oldQty;

                if (diff > 0) { // Increasing quantity
                    if (loadScript.products[pid]['stock'] >= diff) {
                        loadScript.products[pid]['stock'] -= diff;
                        loadScript.orderItems[pid]['orderQty'] = newQty;
                    } else {
                        loadScript.dialogError('Not enough stock.');
                        targetEl.value = oldQty; // Reset input
                        return;
                    }
                } else if (diff < 0) { // Decreasing quantity
                    loadScript.products[pid]['stock'] += Math.abs(diff);
                    loadScript.orderItems[pid]['orderQty'] = newQty;
                }
                ...
                // Update Total Order Amount (without redrawing the whole table which would lose focus)
                loadScript.totalOrderAmount = 0;
                for (const [key, item] of Object.entries(loadScript.orderItems)) {
                    loadScript.totalOrderAmount += item.amount;
                }
                loadScript.updateTotalOrderAmount();

                // Update the amount cell for this row
                let amountCell = targetEl.closest('tr').querySelector('td:nth-child(5)');
                if (amountCell) {
                    amountCell.innerHTML = 'PKR ' + loadScript.addCommas(loadScript.orderItems[pid]['amount'].toFixed(2));
                }
            }
        })
    }
					...
					<tr>
						...
						<td>
                            <div class="btn-group" role="group">
                                <span class="btn btn-default btn-xs quantityUpdateBtn quantityUpdateBtn_minus" data-id="${pid}">
                                    <i class="fa fa-minus"></i>
                                </span>
                                <input type="number" class="form-control qtyInput" value="${orderItem['orderQty']}" data-id="${pid}" style="width: 60px; display: inline-block; text-align: center; height: 22px; padding: 0;">
                                <span class="btn btn-default btn-xs quantityUpdateBtn quantityUpdateBtn_plus" data-id="${pid}">
